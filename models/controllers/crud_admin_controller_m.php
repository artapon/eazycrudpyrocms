...
class ".ucfirst($data['class_prefix'])." extends Admin_Controller
{
	...
	public function create_".$data['module_slug']."()
	{
		...
		if($data['check_db_table_exists_file']['has_file'] == 'has_file')
		{
			...
			$file_detail .= $this->_has_file_create($data);
				
		}else{
					
			$file_detail .= $this->_normal_create($data);
		}
	...
	private function _normal_create($data = array())
	{
		$file_detail = "
	...
	private function _has_file_create($data = array())
	{
		
		$file_detail = "
			\$folder_id = \$this->".$data['module_slug']."_m->get_folder_id();
				
			if(empty(\$folder_id))
			{
				Files::create_folder(0,'".ucfirst($data['module_slug'])." Files');
			}
					
			\$folder_id = \$this->".$data['module_slug']."_m->get_folder_id();
					
			\$results = Files::upload(\$folder_id->id,\$_FILES['".$data['check_db_table_exists_file']['Field']."']['name'],'".$data['check_db_table_exists_file']['Field']."',null,null,null, null);
	
			if(\$id = \$this->".$data['module_slug']."_m->create_".$data['module_slug']."(\$_POST,\$results))
			{
				\$this->session->set_flashdata('success',lang('create_success'));
	...
	public function _edit_function($data = array())
	{
		...
		$file_detail = "public function edit_".$data['module_slug']."(\$id = 0)
		{
			...
			if($data['check_db_table_exists_file']['has_file'] == 'has_file')
			{
				...
				$file_detail .= $this->_has_file_update($data);
				
			}else{
					
				$file_detail .= $this->_normal_update($data);
			}
	...
	private function _normal_update($data = array())
	{
		$file_detail = "
	...
	private function _has_file_update($data = array())
	{
		$file_detail = "
				...
				if(!empty(\$input['".$data['check_db_table_exists_file']['Field']."_id']) )
				{
						
						
					if(!empty(\$_FILES['".$data['check_db_table_exists_file']['Field']."']['name'])){
							
						\$results = Files::replace_file(\$input['".$data['check_db_table_exists_file']['Field']."_id'],\$folder_id->id,\$_FILES['".$data['check_db_table_exists_file']['Field']."']['name'],'".$data['check_db_table_exists_file']['Field']."',null,null,null, null);
							
					}else{
							
						\$results['data']['id'] = \$input['".$data['check_db_table_exists_file']['Field']."_id'];
				...
				if(\$this->".$data['module_slug']."_m->update_".$data['module_slug']."(\$id,\$_POST,\$results))
					{
						\$this->session->set_flashdata('success',lang('edit_success'));
	...
	private function _btn_action($data = array())
	{
		$file_detail = "
				if(\$this->input->post('btnAction') =='save_exit'){
